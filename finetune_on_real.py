...
def main():
    ...
    for epoch in range(args.start_epoch,args.num_epochs_stop):
      train_real_epoch(realloader,model,optimizer,epoch,args,train_writer)
      val_acc,val_loss = val_on_real(testloader,model,epoch,args,train_writer)
      train_writer.add_scalar('finetuning_phase_real_loss', val_loss, epoch)
      train_writer.add_scalar('finetuning_phase_real_acc', val_acc, epoch)
...
def train_real_epoch(loader,model,optimizer,current_epoch,args,log):
  for idx, data in enumerate(loader):
    ...
    src_score, loss_src = model(src_img, lbl=src_lbl,test=False)

    Loss = loss_src.mean()
    Loss.backward()
    optimizer.step()
      # visualize_and_log_depth(src_img[0],trg_img[0],model,current_epoch,train_writer)
    print('[epoch %d][it %d][src loss %.4f][lr %.4f]' % \
          (current_epoch + 1, idx + 1, loss_src.mean().data, 
